<div class="table-responsive">
  ...
  <button type="button" class="btn btn-outline-primary d-inline float-right ml-2" wire:click="downloadList">
    ...
    <span class="d-inline">Pobierz listę pracowników</span>
  </button>

  <button type="button" class="btn btn-outline-primary float-right d-inline" wire:click="$emit('modalWorker')">
    ...
    <span class="d-inline">Nowy pracownik</span>
  </button>

  <table class="table table-hover mt-4">
    <thead>
      @foreach ($headers as $key => $value)
      <th width="14%" style="cursor: pointer" wire:click="sort('{{ $key }}')">
        @if($sortColumn == $key) 
    ...
    <tbody>
      ...
      <tr>
        ...
        @endforeach
        <td>
          <button class="btn btn-outline-warning btn-sm" wire:click="$emit('editWorker', {{ $item['id_w'] }})">
            <span class="d-inline">Edytuj</span>
          </button>
          <button type="button" class="btn btn-outline-danger btn-sm" wire:click="$emit('removeWorker', {{ $item['id_w'] }})">
      ...
      @endforeach
      @else
      <tr><td colspan="{{ count($headers) }}"><h2>Brak pracowników w bazie.</h2></td></tr>
      @endif
